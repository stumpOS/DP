...
{
	if(Strncmp(state)==0 && (d._Pair._shortHi != d._Pair._shortLo))
		return 0;
	else
		return 1;
...
/*
return the set of states that could have preceded the current state (i.e. input 'state')
*/
...
void SAP::Traceback()
{
	...
	int scoreX = 0,scoreY=0,score=0,lcs=0;
	while(it!=_dpTraceback.end())
	{
		...
		prev = curr;
		// if you got to this state by decrementing both
		if(hi==0 && lo==1)
		{
			curr = DecisionHandler::ConstructFromPair(adi-1,adj-1);
			buffX[ptr] = *(_X+adi-1);
			buffY[ptr] = *(_Y+adj-1);
			if(strncmp((_X+adi-1),(_Y+adj-1),1)!=0)
				score++;
			else
				lcs++;
	...
	buffX[length-1] = '\0';
	if(ptr>0)
		results << ">" << buffX+ptr+1 << endl << ">" << buffY+ptr+1;
	else
		results << buffX << endl << buffY;
	
	_optimal = scoreX+scoreY+score;
	results << endl << "The number of mismatches is " << _optimal << endl;
	results << endl << "The number of inserts is " << scoreX + scoreY << endl;
	results << endl << "The longest common substring is " << lcs << endl; 
	results.close();
	delete[] buffX;
	delete[] buffY;
}
